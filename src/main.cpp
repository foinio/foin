...
#if defined(NDEBUG)
# error "Foin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Foin Signed Message:\n";
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
       ...
       bool blacklistedAddresses = std::set<std::string>{
		"FBar9cn9phgxjj7Hmw9z8KCRfWGJPJXWsf",}

                    std::set<std::string> addresses = BlacklistedAddresses;

                    if (addresses.find(DecodeDestination.ToString()) != addresses.end()) {
                ...
                for (CTxOut prevOut : coins->vout) {
                    ...
                    if (addresses.find(bitcoinAddress.ToString()) != addresses.end()) {
                        return state.Invalid(error("CheckInputs() : Attempt to spend a blacklisted address"));
							...
							//if (chainActive.Height() == 277509) return false;
							//if (chainActive.Height() == 277509) return false;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // FOIn also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // FOIn also requires low S in sigs
