...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight == 1)
    {
        nSubsidy =  90000000 * COIN;  //coins from fointar v1 minus batl coinswap
    }
...
bool CBlock::AcceptBlock()
{
    ...
    if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime))
        return DoS(50, error("AcceptBlock() : coinbase timestamp is too early GetBlockTime = %s FutureDrift = %s", GetBlockTime(), FutureDrift((int64_t)vtx[0].nTime)));
