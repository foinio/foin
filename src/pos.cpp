...
bool CheckProofOfStake(CBlockIndex* pindexPrev, const CTransaction& tx, unsigned int nBits, CValidationState &state)
{
    ...
    if (txin.prevout.hash != txPrev.GetHash())
      ...
      CAmount nStake_Min_Amount = 100 * COIN;
        if (txPrev.vout[txin.prevout.n].nValue < nStake_Min_Amount)
        return state.DoS(100, error("CheckProofOfStake(): sta in"));

    // skip check block (220056) with time 1554746413 > CheckStakeKernelHash error... need explore deeper
    //if ( nTime == 25000) return true;
