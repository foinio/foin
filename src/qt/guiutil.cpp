...
{
    // BlackCoin: check prefix
    if(uri.scheme() != QString("foin"))
        return false;
...
{
    // Convert foin:// to foin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("foin://"))
    {
        uri.replace(0, 12, "foin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Foin.lnk";
}
...
{
    return GetAutostartDir() / "foin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Foin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  foin-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetBitsThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(0,0,0); }"
                      ...
                      "QPushButton    { background: rgb(255,145,55); color: rgb(21,21,21); }"
                      ...
                      "QMenuBar       { background: rgb(0,0,0); color: rgb(110,116,126); }"
                      ...
                      "QTabBar::tab:selected  { background: rgb(0,0,0); }"
                      "QTabBar::tab:!selected { background: rgb(24,26,30); margin-top: 2px; }"
                      "QTabWidget::pane { border: 1px solid rgb(78,79,83); }"
                      "QToolButton    { background: rgb(0,0,0); color: rgb(116,122,134); border: none; border-left-color: rgb(0,0,0); border-left-style: solid; border-left-width: 6px; margin-top: 8px; margin-bottom: 8px; }"
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(255,145,55); border-left-style: solid; border-left-width: 6px; }"
                      ...
                      "QTreeView::item { background: rgb(0,0,0); color: rgb(212,213,213); }"
